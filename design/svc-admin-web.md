...
- **☑ M0 — spec sign-off.** Frontend stack pinned (htmx + Alpine.js), open questions resolved. Commit: tbd.
- **☐ M1 — module skeleton.** `service/svc-admin-web` Maven module, parent wired, empty `WebAdminService` with config + lifecycle, served at `/healthz` only.
