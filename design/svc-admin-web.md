...
| `GET`  | `/api/services` | yes | Service inventory — parsed from `server.service.list`; `404` when that command is absent |
| `GET`  | `/api/agents` | yes | Agent groups + processors — parsed from `server.processors.list`; `404` when absent |
...
- **☑ M6 — conditional tabs.** Cache + Loader panels surface as full-width sections when the corresponding commands are present (predicates `hasCacheCommands`, `hasYamlLoader`, `hasSpringLoader` against `/api/commands`). Cache panel exposes `cache.list`, `cache.{name}.keys`, `cache.{name}.get` actions; loader panel exposes `yamlLoader.compileProcessor` and `springLoader.compileProcessor` forms with optional file picker. `GET /api/files` is mounted unconditionally but returns 404 when `loaderBaseDir` is unset (so the "browse…" button hides automatically). Path-traversal hardening: reject absolute paths and `..` segments up front; `toRealPath` + `startsWith(base)` after resolve catches symlink escapes. New `invokeRaw(name, args)` helper isolates panel actions from the main command runner's UI state. 39 tests green (5 new file picker: 404 when unset, lists entries, rejects `..`, rejects absolute, blocked unauth). Commit: `e78b4b0`.
- **☑ M7 — docs + README + screenshots.** Full sibling-style `service/svc-admin-web/README.md` rewritten from M1 stub: feature tour, HTTP/WS surface table, auth model, security model, configuration reference, ops notes, related plugins. Matching mkdocs page at `docs/services/admin-web.md` wired into `mkdocs.yml` nav between `admin-telnet` and `cache`, and added to the services landing-page grid with a `material-monitor-dashboard` icon. Top-level catalogue (`README.md`) gets a row in the services table + tree. Dashboard screenshot captured headless via Chrome at 1280×1000 and referenced from both READMEs. Commit: `4761c90`.

- **☑ M8 — console redesign + dispatcher panels.** SPA rebuilt as a nav-rail admin console: fixed topbar (server identity, WebSocket status pill, theme toggle), left nav rail with one-view-at-a-time routing via Alpine `activeView`, light/dark theme driven by CSS custom properties (persisted to `localStorage`, honours `prefers-color-scheme` before first paint). Dashboard reworked into stat cards + heap usage meter + inline-SVG heap sparkline (rolling `/ws/monitor` samples) + GC table. UX upgrades per §4: connection-status pills, empty states throughout, toast notifications on command/login actions, a real auth-disabled banner, result copy button. New dispatcher views: **Queues** parses the existing `eventSources` built-in command into a per-source consumer topology — no backend change; **Services** and **Agents** are backed by new structured endpoints `GET /api/services` and `GET /api/agents`, which invoke and parse `server.service.list` / `server.processors.list` server-side (registry already injected — no `MongooseServerController` wiring needed). All three dispatcher endpoints `404` when their source command is absent, so the nav items self-hide like the conditional Cache/Loader tabs. Frontend stack unchanged (htmx + Alpine, no node toolchain). 44 tests green (5 new: service-list parse, processor-list parse, both 404-when-absent, services unauth gate).
